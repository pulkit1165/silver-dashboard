      ...
      <section className="panel">
        ...
        <div className="overflow-x-auto">
          <table className="rtable">
            <thead>
              <tr>
                <th>Ref</th><th>Party</th><th>Salesman</th><th>Order date</th><th>Required by</th>
                <th className="!text-right">Items</th><th className="!text-right">Value (MRP)</th><th></th>
              </tr>
            </thead>
            <tbody>
              {orders.length === 0 && (
                <tr><td colSpan={8} className="!py-6 text-center text-[var(--muted)]">No decoded orders — <Link href="/erp/sales/decode/manual" className="font-semibold text-[var(--accent)]">write one</Link>.</td></tr>
              )}
              {orders.map((o) => (
                <tr key={o.id}>
