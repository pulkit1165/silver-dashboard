...
export default function ImportLabelInfo() {
  const [rows, setRows] = useState<Row[]>([]);
  const [fileName, setFileName] = useState("");
  const [preview, setPreview] = useState<{ willUpdate: number; errors: Err[] } | null>(null);
  const [result, setResult] = useState<{ updated: number; skipped: number; errors: Err[] } | null>(null);
  ...
  async function run(dryRun: boolean) {
    ...
    setBusy(dryRun ? "Validating…" : "Updating…");
    try {
      const r = await fetch("/api/erp/skus/import-labels", {
        method: "POST", headers: { "content-type": "application/json" },
        body: JSON.stringify({ rows, dryRun }),
      });
      const d = await r.json();
      if (!d.ok) { alert(d.error || "Failed"); return; }
      if (dryRun) setPreview({ willUpdate: d.willUpdate, errors: d.errors });
      else { setResult({ updated: d.updated, skipped: d.skipped, errors: d.errors }); setPreview(null); }
    } finally {
      setBusy("");
    }
  }
    ...
    <div className="flex flex-col gap-5">
        ...
        <section className="panel">
          ...
          <div className="flex gap-2 border-t border-[var(--border)] p-3">
            ...
            <button onClick={() => run(false)} disabled={!!busy || !preview} className="rounded-lg bg-[var(--accent)] px-4 py-2 text-sm font-bold text-white hover:bg-[var(--accent-strong)] disabled:opacity-50">
              {busy || `Update ${preview ? preview.willUpdate : ""} SKUs`}
            </button>
      ...
      {preview && !result && (
        <section className="panel">
          ...
          <div className="p-4">
            <p className="text-sm"><b className="text-[var(--accent-2)]">{preview.willUpdate}</b> rows will update. <b className="text-[var(--danger)]">{preview.errors.length}</b> skipped.</p>
      ...
      {result && (
        <section className="panel">
          <div className="panel-hd">Update complete</div>
          <div className="p-4">
            <div className="mb-3 rounded-xl border px-4 py-3 text-sm font-bold" style={{ borderColor: "var(--accent-2)", background: "var(--accent-2-bg)", color: "var(--accent-2)" }}>
              ✓ Updated {result.updated} SKU(s). {result.skipped > 0 && `${result.skipped} skipped.`}
            </div>
            {result.errors.length > 0 && <ErrTable errors={result.errors} />}
            <div className="mt-3 flex gap-2">
              <a href="/erp/labels" className="rounded-lg bg-[var(--accent)] px-4 py-2 text-sm font-bold text-white">Print barcode labels →</a>
