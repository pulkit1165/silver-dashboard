...
export const companySettings = pgTable("company_settings", {
  ...
  // Invoice value above which an e-way bill is legally required (₹50,000 by
  // default rule; kept editable in case a state-specific rule ever differs).
  ewbThreshold: doublePrecision("ewb_threshold").default(50000),
});
...
export const invoices = pgTable("invoices", {
  ...
  ewbValidUntil: text("ewb_valid_until").default(""),
  createdBy: text("created_by"),
  createdAt: createdAt(),
});

// GSTR-2B is generated by the government from suppliers' filings, not by us —
// each month someone downloads the JSON from the GST portal and uploads it
// here; gstr2bRecords is its parsed B2B invoice lines, reconciled in-app
// against vendor_bills (see lib/erp/gstr2b.ts). One upload per return period.
export const gstr2bUploads = pgTable("gstr2b_uploads", {
  id: serial("id").primaryKey(),
  period: text("period").notNull(), // "YYYYMM", the GSTR-2B return period
  fileName: text("file_name").default(""),
  uploadedBy: text("uploaded_by"),
  createdAt: createdAt(),
});

export const gstr2bRecords = pgTable(
  "gstr2b_records",
  {
    id: serial("id").primaryKey(),
    uploadId: integer("upload_id").notNull(),
    supplierGstin: text("supplier_gstin").default(""),
    ...
    cgst: doublePrecision("cgst").default(0),
    sgst: doublePrecision("sgst").default(0),
    itcAvailability: text("itc_availability").default(""), // GSTN's own flag, e.g. "Yes"/"No"
    rawJson: jsonb("raw_json"),
  },
  (t) => ({ byUpload: index("gstr2b_upload_idx").on(t.uploadId) }),
);
